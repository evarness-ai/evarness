...
- Planned (tracked, not pretended): a second in-tree example domain proving the
  ...
  the second domain's arrival, not before; `GraphParams` carries the same
  flavor (`context_budget_tokens`, `max_loops`, the `provider` default) — the
  graph schema is hashed into proof subjects, so restructuring these into a
  domain-owned section is part of that same versioned migration, not a quiet
  edit; versioned public-API stability
