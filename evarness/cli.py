...
from .patterns import (fixture_names, fixture_text,
                       invariant_defs as pattern_invariant_defs,
                       list_patterns, load_pattern)
from .prove import (prove, render_junit, render_proof_html, render_sarif,
                    verify_proof)
...
def cmd_run(args) -> int:
    ...
    try:
        run = execute(graph, fixture, user_input=args.input, approvals=approvals,
    ...
    if args.json:
        print(json.dumps({"id": run.id, "status": run.status, "output": run.output,
                          "totals": run.totals, "trace_digest": digest,
                          "invariants": run.invariants,
                          "events": run.events}, indent=2))
    else:
        for ev in run.events:
            print(f"{ev['seq']:>3}  {ev['type']:<22} {ev['node_id'] or '':<6} "
    ...
    if args.trace_out:
        meta = {"run_id": run.id, "name": graph.name, "graph_id": graph.id,
                "status": run.status, "reason": run.reason,
                "seed": graph.params.seed, "provider": graph.params.provider,
                "fixture": fixture.scenario, "trace_digest": digest}
...
def cmd_prove(args) -> int:
    ...
    if pattern_doc:
        ...
        inv_defs = (yaml.safe_load(inv_file.read_text()) or {}).get("invariants") \
            if inv_file.exists() else None
        pattern_id = None

    log_activity("cli.prove", graph.id, actor="cli",
                 scenarios=[n for n, _ in scenarios])
    proof = prove(graph, scenarios, pattern_id=pattern_id,
                  invariant_defs=inv_defs, approvals=approvals,
    ...
    if args.sign:
        ...
        if att["key_created"]:
            print(f"new signing key created at {att['key_path']} "
    ...
    for sc in proof["scenarios"]:
        ...
        print(f"  {sc['fixture']:<16} {sc['status']:<10} invariants {inv_txt:<8} "
              f"{repro:<15} {sc['trace_digest'][:24]}\u2026")
    print(f"PROOF: {'HOLDS' if v['ok'] else 'FAILED'} \u2014 "
...
def cmd_verify(args) -> int:
    ...
    result = verify_proof(doc, pubkey_b64=pubkey,
    ...
    print(f"VERIFY: {'OK' if result['ok'] else 'FAILED'} — bundle is "
          + ("internally consistent"
             if result["ok"] else "inconsistent or tampered; see ✗ above"))
    print("note: verification proves consistency and (if signed) integrity — "
...
def _print_verdicts(inv: dict) -> None:
    print(f"INVARIANTS: {inv['passed']} passed, {inv['failed']} FAILED"
          if inv["failed"] else f"INVARIANTS: {inv['passed']} passed")
    for r in inv["results"]:
        mark = "✓" if r["ok"] else "✗"
        where = f" (seq {', '.join(map(str, r['evidence_seq']))})" \
...
def cmd_patterns(args) -> int:
    for p in list_patterns():
        print(f"{p['id']:<28} {p['name']:<34} [{p['source']:<7}] "
...
def main(argv: list[str] | None = None) -> int:
    init_db()
    ap = argparse.ArgumentParser(prog="evarness",
                                 description="Evarness engine — run harness graphs headless")
    ...
    r.add_argument("--approve", action="append", metavar="GATE=approve|reject",
    ...
    r.add_argument("--trace-format", default="jsonl",
                   help="trace export format for --trace-out (jsonl, otlp, or a "
                        "plugin-registered format)")
    ...
    pv.add_argument("--fixture", action="append",
                    help="fixture name (pattern) or YAML path (graph file); repeatable; "
                         "default: all of the pattern's fixtures")
    pv.add_argument("--approve", action="append", metavar="GATE=approve|reject",
    ...
    pv.add_argument("--no-events", action="store_true",
                    help="omit the canonical event streams (smaller bundle)")
    pv.add_argument("--junit", metavar="PATH",
                    help="also write the verdicts as JUnit XML (CI test report)")
    pv.add_argument("--sarif", metavar="PATH",
                    help="also write the violations as SARIF 2.1.0 (code-scanning report)")
    pv.add_argument("--sign", action="store_true",
                    help="attach an Ed25519 attestation (key auto-created on first "
                         "use; needs the [sign] extra)")
    pv.add_argument("--key", metavar="PATH",
                    help="signing key path (default ~/.evarness/keys/proof_ed25519.pem)")
    pv.set_defaults(fn=cmd_prove)

    vf = sub.add_parser("verify", help="re-check a proof bundle: digests, chains, verdict, signature")
