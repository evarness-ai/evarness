...
NODE_PRESENTATION: dict[str, tuple[str, str]] = {
    ...
    "prompt_template": ("📝", "Prompt Template"), "llm": ("🧠", "LLM"),
    "output_parser": ("✂️", "Output Parser"), "loop_controller": ("🔁", "ReAct Loop"),
    "intent_router": ("🚦", "Intent Router"), "interceptor": ("🛂", "Interceptor"),
    "data_classifier": ("🏷️", "Data Classifier"), "tier_router": ("🎚️", "Tier Router"),
    "approval_gate": ("✋", "Approval Gate"), "judge_chain": ("⚖️", "Judge Chain"),
    "llm_guard": ("🛡️", "LLM Guard"), "llm_judge": ("⚖️", "LLM Judge"),
    "redaction_rules": ("🧼", "Redaction"), "policy_gate": ("📜", "Policy Gate"),
    "rate_budget_limiter": ("💰", "Budget Limiter"), "tool": ("🔧", "Tool"),
    "retriever": ("🔎", "Retriever"), "conversation_buffer": ("💬", "Conversation Buffer"),
    "working_memory": ("🗒️", "Working Memory"), "episodic_memory": ("📚", "Episodic Memory"),
    "semantic_memory": ("👤", "User Profile"), "procedural_memory": ("📋", "Standing Instructions"),
    "summary_consolidator": ("🗜️", "Summary Consolidator"),
    "context_assembler": ("🧩", "Context Assembler"), "trace_probe": ("👁️", "Trace Probe"),
    "metrics_emitter": ("📈", "Metrics"), "cost_latency_monitor": ("⏱️", "Cost & Latency"),
    "audit_log_sink": ("🧾", "Audit Sink"),
}

GROUP_ORDER = ["core", "governance", "tools", "rag", "context", "memory", "observability"]
GROUP_TITLES = {"core": "Core", "governance": "Governance", "tools": "Tools",
                "rag": "Retrieval (RAG)", "context": "Context", "memory": "Memory",
...
def _tool_spec_gate(tool: str, cfg, node_id: str):
    ...
    spec = tool_spec(tool)
    if (spec and spec.safety.approval_required()
            and not getattr(cfg, "approve_side_effects", False)):
        raise NodeBlocked(node_id, f"tool '{tool}' has {spec.safety.side_effects} "
                          "side effects and requires approval — enable "
                          "'approve_side_effects' on the node to opt in")
    return spec


def _run_real_tool_contained(tool: str, query: str, cfg, spec, node_id, ctx,
                             sandbox_value: str):
    """Execute a real tool at the requested containment level. `off` runs
    in-process (historical); `subprocess`/`strict` run the executor in a child.
    A requested-but-unavailable level is a NodeBlocked (governance honesty); a
    tool killed by an active sandbox is a normal ToolError, raised to the caller.
    Emits `sandbox_applied` so the trace records what actually contained the run.

    a `network: outbound` tool with `egress: gateway` under strict
    runs behind a hostname-allowlisting proxy — the OS denies every other
    destination. `egress_allowed`/`egress_denied` events record what the proxy
    let through and refused."""
    from .sandbox import (SandboxError, SandboxUnavailable, resolve_level,
                          run_sandboxed)
    from .vault import resolver_scope
    level = resolve_level(sandbox_value)
    if level == "off":
        with resolver_scope(ctx.graph.id, node_id):
            from .tools import run_real_tool
            return run_real_tool(tool, query, cfg)
    try:
        from .sandbox import ensure_available
        ensure_available(level)
    except SandboxUnavailable as exc:
        raise NodeBlocked(node_id, str(exc))

    egress_mode = str(getattr(cfg, "egress", "") or "").strip().lower()
    want_egress = (egress_mode == "gateway"
                   and spec is not None
                   and getattr(spec.safety, "network", "outbound") == "outbound")
    if egress_mode == "gateway" and not want_egress:
        # asked for filtered egress on a tool that declares no outbound network:
        # honest refusal, not a silent no-op
        raise NodeBlocked(node_id, f"tool '{tool}' has egress:gateway but its "
                          "manifest declares no outbound network — nothing to filter")
    if want_egress and level != "strict":
        raise NodeBlocked(node_id, "egress:gateway requires sandbox:strict "
                          "(the OS must deny direct network for the proxy to be "
                          "authoritative) — set sandbox:strict on this node")

    with resolver_scope(ctx.graph.id, node_id):     # scopes which secrets are passed
        if want_egress:
            from .sandbox import run_egress_gateway
            allow_hosts = list(getattr(cfg, "allow_hosts", None) or [])
            ctx.emit("sandbox_applied", node_id, tool=tool, level="strict",
                     egress="gateway", allow_hosts=allow_hosts)
            try:
                docs, audit = run_egress_gateway(tool, query, cfg, spec,
                                                 allow_hosts, ctx=ctx)
            except SandboxUnavailable as exc:
                raise NodeBlocked(node_id, str(exc))
            ...
            for a in audit:
                ctx.emit("egress_allowed" if a["allowed"] else "egress_denied",
                         node_id, host=a["host"], reason=a["reason"])
            return docs
        ctx.emit("sandbox_applied", node_id, tool=tool, level=level)
        try:
            return run_sandboxed(level, tool, query, cfg, spec, ctx=ctx)
        except SandboxError as exc:
            from .tools import ToolError
            raise ToolError(str(exc))
...
def _egress_gate(ctx, node_id: str, destination: str) -> None:
    ...
    ctx.emit("egress_checked", node_id, destination=destination,
             classification=classification,
             verdict="allow" if allowed else "deny", mode=mode)
    if not allowed:
        ctx.emit("egress_denied", node_id, destination=destination,
                 classification=classification,
                 action="block" if mode == "enforce" else "warn")
        if mode == "enforce":
            raise NodeBlocked(node_id, f"egress denied: {classification} content "
...
def _doc_previews(result: list, cap: int = 12) -> list[dict]:
    ...
    return [{"id": str(d.get("id", ""))[:200],
             "subject": str(d.get("subject", ""))[:100]} for d in result[:cap]]
...
def _warn_free_search(node_id: str, tool: str, mode: str, provider: str, ctx) -> None:
    ...
    if p is not None and p.best_effort:
        ctx.emit("tool_warning", node_id, tool=tool, provider=p.name,
...
class InputNode(NodeSpec):
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ctx.emit("input_received", node_id, text=ctx.user_input, tokens=approx_tokens(ctx.user_input))
...
class PromptTemplateNode(NodeSpec):
    ...
    group = "core"
    doc = ("Renders the final prompt text from the assembled context via an editable "
...
class LLMNode(NodeSpec):
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        prompt = (payload.get("prompt") or as_text(payload)) \
            if isinstance(payload, dict) else as_text(payload)
        if cfg.system:
            prompt = f"{cfg.system}\n\n{prompt}"
        _egress_gate(ctx, node_id, _provider_locality(ctx))   # before the model boundary
        ptok = approx_tokens(prompt)
        ctx.emit("llm_request", node_id, provider=ctx.params.provider,
                 temperature=cfg.temperature, max_tokens=cfg.max_tokens, prompt_tokens=ptok)
        try:
            completion = ctx.provider.complete(prompt, temperature=cfg.temperature,
        ...
        ctx.emit("llm_response", node_id, tokens=rtok, cost_usd=completion.cost_usd,
...
class OutputParserNode(NodeSpec):
    ...
    group = "core"
    doc = ("Normalizes model output into the declared output type. format=json "
...
class IntentRouterNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Deterministic, non-LLM routing. Unmatched intents never reach the model — "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if cfg.fallback == "reject":
            raise NodeBlocked(node_id, "no intent rule matched; deterministic router rejected the input")
        return text
...
class InterceptorNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Pure-Python middleware at a node boundary: pii_scrub, policy_check, "
...
class DataClassifierNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Tags flowing content public/internal/personal/secret (registered "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.classification = max_class(getattr(ctx, "classification", "public"),
                                       classification)
        ...
        ctx.emit("content_classified", node_id,
                 classifier=cfg.classifier if not unknown else "keyword",
                 classification=classification, signals=signals[:8],
                 run_classification=ctx.classification, egress=ctx.egress_mode,
...
class ApprovalGateNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Human-in-the-loop checkpoint: PAUSES the run (never silently fails) "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if cfg.require_when == "classified":
            ...
            needs = True
        if not needs:
            ctx.emit("approval_skipped", node_id, require_when=cfg.require_when,
                     classification=classification)
            return text
        ...
        if decision is None:
            ...
            ctx.emit("approval_requested", node_id, prompt=cfg.prompt,
                     preview=text[:120], classification=classification)
...
class TierRouterNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Routes the turn to a model tier by intent (tiers + intent map in "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        from .tiers import (fallback_tier, resolve_tier, tier_locality,
                            tier_provider)
        ...
        if egress_on and not egress_allowed(classification, locality):
            if cfg.on_forbidden_egress == "block":
                ctx.emit("egress_denied", node_id, destination=locality,
                         classification=classification, action="block")
                raise NodeBlocked(node_id, f"tier '{tier}' ({locality}) is egress-"
                                  f"forbidden for {classification} content")
            if cfg.on_forbidden_egress == "warn":
                ctx.emit("tier_egress_warning", node_id, tier=tier, locality=locality,
                         classification=classification)
                ...
                if egress_on and not egress_allowed(classification, fb_loc):
                    ...
                    ctx.emit("egress_denied", node_id, destination=fb_loc,
                             classification=classification, action="block")
                    raise NodeBlocked(node_id, f"no egress-legal tier for "
                                      f"{classification} content — redact before routing")
                ctx.emit("tier_downshifted", node_id, **{"from": tier}, to=fb,
                         reason=f"egress_{classification}", classification=classification)
        ...
        ctx.emit("tier_selected", node_id, intent=intent, tier=tier, provider=spec,
                 locality=locality, reason=reason,
...
class ToolNode(NodeSpec):
    type_name = "tool"
    group = "tools"
    doc = ("Tool with a sim twin (fixture-scripted) and real twins behind the same "
    ...
    class Config(BaseModel):
        ...
        retries: int = 2
        allow_hosts: list[str] = Field(default_factory=list)   # web.fetch allowlist / web.search scope
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("tool_called", node_id, tool=cfg.tool, query=query[:120], mode=cfg.mode,
                 **({"tool_version": spec.version} if spec else {}))
        _warn_free_search(node_id, cfg.tool, cfg.mode,
                          getattr(cfg, "search_provider", ""), ctx)
        sim_default = False
        try:
            if cfg.mode == "real":
                result = _run_real_tool_contained(
                    cfg.tool, query, cfg, spec, node_id, ctx,
                    getattr(cfg, "sandbox", ""))
            else:
            ...
            return []
        ctx.emit("tool_result", node_id, tool=cfg.tool, count=len(result),
...
class RetrieverNode(NodeSpec):
    type_name = "retriever"
    group = "rag"
    doc = ("Scores and filters candidate documents (SimVectorStore in v1; "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("retrieval_performed", node_id, top_k=cfg.top_k, candidates=len(docs),
                 returned=len(chunks), scores=[d["_score"] for d in chunks])
        return chunks
...
class ContextAssemblerNode(NodeSpec):
    ...
    group = "context"
    doc = ("Token-budget-aware prompt assembly. Its context_snapshot event powers "
    ...
    class Config(BaseModel):
        ...
        overflow: Literal["truncate_retrieved", "summarize_retrieved",
                          "drop_lowest_score", "error"] = "truncate_retrieved"
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        def doc_tokens(docs):
            return sum(approx_tokens(f"{d.get('subject','')} {d.get('snippet', d.get('text',''))}") for d in docs)

        seg = lambda kind, tok: {"kind": kind, "tokens": tok}
        segments = [seg("system", approx_tokens(cfg.system)),
        ...
        while total > budget and documents and cfg.overflow == "truncate_retrieved":
            ...
            total = sum(s["tokens"] for s in segments)
        ctx.emit("context_snapshot", node_id, segments=segments, total_tokens=total,
                 budget=budget, truncated_docs=truncated)
        if total > budget:
            ...
            ctx.emit("budget_breached", node_id, total_tokens=total, budget=budget,
                     over_by=total - budget)
        return {"system": cfg.system, "question": question,
                "documents": documents, "memory": memory, "segments": segments}
...
class LLMGuardNode(NodeSpec):
    type_name = "llm_guard"
    group = "governance"
    doc = ("Probabilistic guard — a classifier, i.e. a model guarding a model. Real "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        text = as_text(inputs.get("in", ""))
        low = text.lower()
        detections = [a for a in ctx.fixture.attacks
                      if str(a.get("marker", "")).lower() in low
                      and (a.get("type", "prompt_injection") in cfg.detects
                           or "all" in cfg.detects)]
        ...
        ctx.emit("guard_evaluated", node_id, detections=len(detections), score=round(score, 4),
                 triggered=triggered, missed=missed, false_positive=false_positive)
        if triggered:
            ...
            if cfg.on_trigger == "block":
                raise NodeBlocked(node_id, f"llm_guard triggered on {'/'.join(kinds)}"
...
class LLMJudgeNode(NodeSpec):
    type_name = "llm_judge"
    group = "governance"
    doc = ("LLM-as-judge scoring node (sim:judge-v1). Emits judge_scored events the "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if mean < cfg.threshold:
            ...
            if cfg.on_fail == "block":
                raise NodeBlocked(node_id, f"llm_judge score {mean} below threshold {cfg.threshold}")
        return text
...
class JudgeChainNode(NodeSpec):
    ...
    group = "governance"
    doc = ("An ordered curation chain: several registered judges run in "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        for name in cfg.judges:
            ...
            while True:
                ...
                ctx.emit("judge_signal", node_id, judge=name, verdict=sig.verdict,
            ...
            if verdict == "retry":   # budget exhausted without a pass
                ...
                ctx.emit("judge_exhausted", node_id, judge=name,
                         attempts=attempts, downgraded_to=verdict)
            if verdict == "halt":
                ctx.emit("chain_halted", node_id, judge=name, reason=sig.reason)
                raise NodeBlocked(node_id, f"judge '{name}' halted the response: "
                                  f"{sig.reason or 'unsafe'}")
            if verdict == "warn":
                banners.append(sig.reason or f"{name} flagged")
            verdicts.append({"judge": name, "verdict": verdict})

        ctx.emit("judge_chain_finished", node_id, verdicts=verdicts,
                 banners=banners, passed=not banners)
        if banners:
...
class RedactionRulesNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Deterministic redaction — pure rules, provable, unit-testable. Emits "
...
class PolicyGateNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Org-policy allow/deny rules evaluated deterministically from config — the "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("policy_checked", node_id, verdict="deny" if hit else "allow",
                 mode=cfg.mode, **({"term": hit} if hit else {}))
...
class RateBudgetLimiterNode(NodeSpec):
    ...
    group = "governance"
    doc = ("Hard resource ceilings checked at this point in the flow. Emits "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("budget_checked", node_id, tokens=tokens, tool_calls=tool_calls,
                 max_tokens=cfg.max_tokens, max_tool_calls=cfg.max_tool_calls,
                 within=not breach)
        if breach:
            ctx.emit("budget_breached", node_id, tokens=tokens, tool_calls=tool_calls,
                     action=cfg.on_breach)
            if cfg.on_breach == "abort":
                raise NodeBlocked(node_id, f"budget breached: {tokens} tokens, "
...
_GOVERNANCE_EVENTS = {"intent_routed", "interceptor_applied", "policy_violation",
                      "redaction_applied", "guard_evaluated", "guard_triggered",
                      "judge_scored", "judge_flagged", "policy_checked",
                      "budget_checked", "budget_breached",
                      # classification + egress, tier routing, approvals
                      "content_classified", "egress_checked", "egress_denied",
                      "tier_selected", "tier_downshifted", "tier_egress_warning",
                      "approval_requested", "approval_granted", "approval_rejected",
                      "approval_skipped", "run_paused",
                      # judge chain
                      "judge_signal", "judge_repaired", "judge_exhausted",
                      "judge_degraded", "chain_halted", "judge_chain_finished"}
...
class TraceProbeNode(NodeSpec):
    ...
    group = "observability"
    doc = ("Taps an edge and snapshots what flows through it. Zero effect on "
...
class MetricsEmitterNode(NodeSpec):
    ...
    group = "observability"
    doc = ("Turns the trace so far into named metrics that Experiments can aggregate "
           "and compare.")
    inputs: ClassVar[dict] = {"in": "any"}
    outputs: ClassVar[dict] = {"out": "any"}
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("metrics_emitted", node_id,
                 **{m: counts[m] for m in cfg.metrics if m in counts})
        return inputs.get("in")
...
class CostLatencyMonitorNode(NodeSpec):
    ...
    group = "observability"
    doc = ("Watches cumulative cost/latency at this point in the run. Sim latency is "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("cost_latency", node_id, cost_usd=cost, sim_latency_ms=latency,
                 within=not over)
        if over:
            ctx.emit("budget_breached", node_id, cost_usd=cost, sim_latency_ms=latency,
                     action=cfg.action)
            if cfg.action == "abort":
                raise NodeBlocked(node_id, f"cost/latency budget breached "
                                  f"(${cost}, {latency}ms simulated)")
        return inputs.get("in")
...
class AuditLogSinkNode(NodeSpec):
    ...
    group = "observability"
    doc = ("Compliance export point. The event trace IS the audit log; this node "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("audit_flushed", node_id, include=cfg.include, format=cfg.format,
                 events_shipped=shipped, governance_events=gov)
        return inputs.get("in")
...
class WorkingMemoryNode(NodeSpec):
    ...
    group = "memory"
    doc = ("Run-scoped scratchpad the harness reads/writes mid-execution — plans, "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if value is not None:
            ...
            ctx.emit("memory_write", node_id, store="working", key=cfg.key,
...
class EpisodicMemoryNode(NodeSpec):
    ...
    group = "memory"
    doc = ("Long-term record of past interactions (fixture `episodic` section). "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        entries = [e for e in ctx.fixture.episodic
                   if float(e.get("salience", 1.0)) >= cfg.min_salience]
        n = len(entries) or 1
        scored = sorted(
            ({**e, "_score": round(SimVectorStore.score(query, {"text": e.get("text", "")})
                                   + (i + 1) / n * 0.1, 4)}   # recency bonus, newest last
             for i, e in enumerate(entries)),
            key=lambda e: (-e["_score"], str(e.get("ts", ""))))[:cfg.top_k]
        ctx.emit("memory_read", node_id, store="episodic", candidates=len(entries),
                 returned=len(scored), scores=[e["_score"] for e in scored])
        if cfg.write_policy != "off":
            salience = round(min(1.0, approx_tokens(query) / 50), 4)
            ctx.emit("memory_write", node_id, store="episodic",
                     accepted=salience >= cfg.write_salience, salience=salience,
                     persisted=False)  # sim: the write path is traced, not persisted
        return [{"role": "episodic", "text": e.get("text", ""), "ts": e.get("ts")}
                for e in scored]
...
class SemanticMemoryNode(NodeSpec):
    ...
    group = "memory"
    doc = ("Distilled facts about the user (fixture `facts` section) — the profile "
        ...
        for f in ctx.fixture.facts:
            ...
            if key in facts:
                ...
                if cfg.conflict == "first_wins":
                    continue
            facts[key] = text
        ctx.emit("memory_read", node_id, store="semantic", facts=len(facts),
                 **({"conflicts": conflicts} if conflicts else {}))
        value = inputs.get("in")
        if value is not None:
            raw = as_text(value)
            written, redactions = (redact_pii(raw) if cfg.redact_before_write else (raw, 0))
            ctx.emit("memory_write", node_id, store="semantic", redactions=redactions,
                     preview=written[:80], persisted=False)
...
class ProceduralMemoryNode(NodeSpec):
    ...
    group = "memory"
    doc = ("Standing instructions & learned behaviors (fixture `instructions` section). "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if pending and cfg.write_approval == "auto":
            ctx.emit("memory_write", node_id, store="procedural", auto_approved=len(pending),
        ...
        ctx.emit("memory_read", node_id, store="procedural",
                 instructions=len(approved), pending=len(pending))
...
class SummaryConsolidatorNode(NodeSpec):
    ...
    group = "memory"
    doc = ("The short-term -> long-term bridge: compresses messages into a summary "
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        if not isinstance(messages, list):
            messages = [messages]
        in_tokens = sum(approx_tokens(m.get("text", "") if isinstance(m, dict) else m)
                        for m in messages)
        firsts = [(m.get("text", "") if isinstance(m, dict) else str(m)).split(".")[0].strip()
                  for m in messages]
        summary = ". ".join(f for f in firsts if f)[:cfg.max_tokens * 4]
        out_tokens = approx_tokens(summary)
        ctx.emit("memory_consolidated", node_id, target=cfg.target, in_tokens=in_tokens,
...
class LoopControllerNode(NodeSpec):
    ...
    group = "core"
    doc = ("ReAct tool loop: the model decides which tool to call, the harness owns "
    ...
    def _model_decision(cls, ctx, node_id, cfg, transcript):
        ...
        prompt = ((f"{cfg.system}\n\n" if cfg.system else "")
                  + cls.REACT_PROTOCOL.format(tools=", ".join(cfg.tools),
        ...
        ctx.totals["cost_usd"] = round(ctx.totals.get("cost_usd", 0.0)
                                       + completion.cost_usd, 6)
        text = completion.text.strip()
        # models mix lines ("FINAL: I don't know.\nTOOL: ..." — found live):
        # a TOOL line ANYWHERE wins — the intent to gather evidence beats the hedge
        tool_line = next((ln.strip() for ln in text.splitlines()
                          if ln.strip().upper().startswith("TOOL:")), None)
        if tool_line:
            ...
            rule = {"respond": {"text": text[6:].strip()
                                if text.upper().startswith("FINAL:") else text}}
            kind, preview = "final", text
        ctx.emit("llm_response", node_id, tokens=completion.output_tokens,
                 cost_usd=completion.cost_usd, preview=preview[:120], decision=kind)
        return rule
    ...
    def run(cls, node_id, inputs, cfg, ctx):
        ...
        ctx.emit("loop_started", node_id, max_iterations=cfg.max_iterations,
                 tools=cfg.tools, scripted=scripted,
                 **({"requested_count": wanted,
                     "max_results": cfg.max_results} if wanted else {}))
        ...
        for i in range(cfg.max_iterations):
            ...
            ctx.emit("llm_request", node_id, provider=ctx.params.provider,
                     purpose="react_decision", iteration=i, prompt_tokens=ptok)
            ...
            if scripted:
                ...
                if rule is None:  # off-script: the honest "I don't know", not fake competence
                    ...
                    ctx.emit("llm_response", node_id, tokens=approx_tokens(answer),
                             preview=answer[:120], decision="final_default")
            ...
            if "respond" in rule:
                ...
                if (not scripted and cfg.require_evidence and not seen_actions
                        and not refused_final):
                    refused_final = True
                    ctx.emit("loop_guard", node_id, reason="final_without_evidence",
                ...
                if not scripted and cfg.grounding_check != "off" and seen_actions:
                    g_ctx = {"question": question, "queries": queries_run,
                             "observations": observations}
                    unsupported, unknown_rules = check_grounding(
                        answer, transcript, cfg.grounding_rules, g_ctx)
                    refuse = bool(unsupported and cfg.grounding_check == "retry"
                                  and grounding_retries_used < cfg.grounding_retries)
                    ctx.emit("grounding_checked", node_id, mode=cfg.grounding_check,
                             verdict="unsupported" if unsupported else "grounded",
                             unsupported=unsupported[:8], refused=refuse,
                    ...
                    if refuse:
                        ...
                        topics = [v.split("'")[1] for v in unsupported
                                  if v.startswith("request topic '")]
                        if topics:
                ...
                if scripted:
                    ctx.totals["tokens"] += ptok + approx_tokens(answer)
                    ctx.emit("llm_response", node_id, tokens=approx_tokens(answer),
                             preview=answer[:120], decision="final")
            ...
            if scripted:
                ...
                ctx.emit("llm_response", node_id, tokens=approx_tokens(decision),
                         preview=decision, decision="tool_call")
            ...
            if tool not in cfg.tools:  # the model asked; the harness refuses
                ...
                raise NodeBlocked(node_id, f"loop_controller: model requested tool "
                                  f"'{tool}' outside allowed tools {cfg.tools}")
            if cfg.dedup_guard and (tool, tool_input) in seen_actions:
                ctx.emit("loop_guard", node_id, reason="repeated_action",
            ...
            ctx.emit("tool_called", node_id, tool=tool, query=tool_input[:120],
                     iteration=i, mode=cfg.tool_mode,
                     **({"tool_version": spec.version} if spec else {}))
            tool_err = None
            sim_default = False
            try:
                if cfg.tool_mode == "real":
                    result = _run_real_tool_contained(
                        tool, tool_input, cfg, spec, node_id, ctx,
                        getattr(cfg, "sandbox", ""))
                else:
                ...
                ctx.emit("tool_error", node_id, tool=tool, mode=cfg.tool_mode,
                         error=str(exc))
                result, tool_err = [], str(exc)
            ctx.emit("tool_result", node_id, tool=tool, count=len(result),
                     docs=_doc_previews(result),
                     **({"sim_default": True} if sim_default else {}))
            # the model must see WHY a call failed, not a bare "no results" —
            # otherwise it can't distinguish "nothing exists" from "bad call"
            obs = "; ".join(f"{d.get('subject', '')}: {d.get('snippet', d.get('text', ''))}"
                            for d in result) or (f"tool error: {tool_err}" if tool_err
                                                 else "no results")
            ...
            if (not result and not scripted and tool.endswith(".search")
                    and i + 1 < cfg.max_iterations):
                ctx.emit("loop_guard", node_id, reason="empty_search_retry",
                         tool=tool, query=tool_input[:120])
                transcript += "\n" + NUDGES["empty_search_retry"].format(tool=tool)

        ctx.emit("loop_finished", node_id, iterations=iterations, stopped=stopped,
                 transcript_tokens=approx_tokens(transcript))
        if answer is None:
            if cfg.on_stop == "fail":
                raise NodeBlocked(node_id, f"loop_controller stopped without an answer "
            ...
            if cfg.on_stop == "best_effort" and not scripted:
                ...
                if answer and cfg.grounding_check != "off" and seen_actions:
                    unsupported, unknown_rules = check_grounding(
                        answer, transcript, cfg.grounding_rules,
                        {"question": question, "queries": queries_run,
                         "observations": observations})
                    ctx.emit("grounding_checked", node_id, mode="warn",
                             verdict="unsupported" if unsupported else "grounded",
                             unsupported=unsupported[:8], refused=False,
                             rules=cfg.grounding_rules,
                             **({"unknown_rules": unknown_rules} if unknown_rules else {}))
            if answer is None:
                tried = ", ".join(f"{t}({q[:40]})" for t, q in sorted(seen_actions)) or "no tools"
                answer = (f"I could not reach a final answer ({stopped} after "
    ...
    def _best_effort(cls, ctx, node_id, cfg, transcript, stopped) -> str | None:
        prompt = ((f"{cfg.system}\n\n" if cfg.system else "")
                  + PROTOCOLS["best_effort_wrapup"].format(stopped=stopped,
                                                           transcript=transcript))
        ctx.emit("llm_request", node_id, provider=ctx.params.provider,
                 purpose="best_effort_answer", prompt_tokens=approx_tokens(prompt))
        try:
        ...
        if text.upper().startswith("FINAL:"):
            text = text[6:].strip()
        ctx.emit("llm_response", node_id, tokens=completion.output_tokens,
                 cost_usd=completion.cost_usd, preview=text[:120],
