...
def test_flagship_happy_path():
    ...
    for expected in ["run_started", "intent_routed", "interceptor_applied", "tool_called",
                     "retrieval_performed", "memory_read", "context_snapshot",
                     "prompt_assembled", "llm_request", "llm_response", "run_finished"]:
...
def test_prompt_vocabulary_is_yaml_derived():
    ...
    from evarness.nodes import (DEFAULT_AGENT_SYSTEM, DEFAULT_LLM_SYSTEM,
                                  LoopControllerNode, PROMPT_TEMPLATES)
...
def test_config_schema_carries_enums():
    ...
    assert props["overflow"]["enum"] == ["truncate_retrieved", "summarize_retrieved",
                                         "drop_lowest_score", "error"]
...
def test_output_parser_json_format_is_real():
    ...
    ctx = _EmitCtx()
    out = OutputParserNode.run("n1", {"in": 'Here you go:\n```json\n{"days": 12,\n "unit": "vacation"}\n```'}, cfg, ctx)
...
def test_semantic_memory_conflict_policy_is_real():
    ...
    fx = Fixture({"facts": [{"fact": "prefers mornings", "key": "meeting_pref"},
...
def test_prompt_template_custom_layout_end_to_end():
    ...
    assert out["prompt"] == ("INSTRUCTIONS: You are helpful.\n"
                             "Context documents:\n[1] PTO: 12 days\n"
...
def _egress_graph(provider: str, egress: str = "enforce") -> GraphModel:
    return GraphModel.model_validate({
        "ir_version": 1, "id": "egress-test", "name": "t", "description": "d44",
        "nodes": [
            {"id": "a", "type": "input", "config": {}, "position": {"x": 0, "y": 0}},
            {"id": "b", "type": "data_classifier",
             "config": {"egress": egress}, "position": {"x": 1, "y": 0}},
...
def test_egress_warn_traces_without_blocking():
    run = execute(_egress_graph("sim:helpful-v1", egress="warn"),
                  load_fixture({"scenario": "t", "user_input": "check api_key=sk-live-99",
...
def _judge_graph(judges, retry_budget=1):
    return GraphModel.model_validate({
        "ir_version": 1, "id": "jc", "name": "t", "description": "d47",
        "nodes": [
            {"id": "a", "type": "input", "config": {}, "position": {"x": 0, "y": 0}},
            {"id": "j", "type": "judge_chain",
...
def test_judge_chain_safety_halts_first():
    run = execute(_judge_graph(["safety", "faithfulness"]),
...
def test_judge_chain_schema_retry_repairs_within_budget():
    run = execute(_judge_graph(["schema"]),
...
def test_judge_chain_retry_exhausts_to_halt():
    fx = load_fixture({"user_input": "the answer is Berlin",
...
def test_judge_chain_timeout_fails_open():
    fx = load_fixture({"user_input": "anything",
                       "faults": {"judge_timeout": ["faithfulness"]}})
    run = execute(_judge_graph(["faithfulness"]), fx)
...
def _approval_graph() -> GraphModel:
    return GraphModel.model_validate({
        "ir_version": 1, "id": "ap", "name": "t", "description": "d46",
        "nodes": [
            {"id": "a", "type": "input", "config": {}, "position": {"x": 0, "y": 0}},
            {"id": "g", "type": "approval_gate",
             "config": {"prompt": "Do it?"}, "position": {"x": 1, "y": 0}},
            {"id": "o", "type": "output", "config": {}, "position": {"x": 2, "y": 0}}],
...
def test_approval_gate_require_when_skips_public():
    ...
    graph = GraphModel.model_validate({
        "ir_version": 1, "id": "ap2", "name": "t", "description": "d",
        "nodes": [
            {"id": "a", "type": "input", "config": {}, "position": {"x": 0, "y": 0}},
            {"id": "c", "type": "data_classifier",
             "config": {"egress": "off"}, "position": {"x": 1, "y": 0}},
            {"id": "g", "type": "approval_gate",
             "config": {"require_when": "classified"}, "position": {"x": 2, "y": 0}},
...
_SEARXNG_JSON = json.dumps({"results": [
...
def test_tool_side_effects_require_node_approval(tmp_path, monkeypatch):
    ...
    manifest.write_text(yaml.safe_dump({"tools": [
        {"id": "acme.post", "description": "posts things",
         "category": "comms", "safety": {"side_effects": "write"}}]}))
    monkeypatch.setenv("EVARNESS_TOOLS", str(manifest))

    fx = load_fixture({"tools": {"acme.post": [
    ...
    g2 = chain(("tool", {"tool": "acme.post", "mode": "sim",
...
def graph_of(nodes, edges, **params):
    return GraphModel.model_validate({"id": "t", "nodes": nodes, "edges": edges,
                                      "params": params})
...
def chain(*types_and_cfg):
    ...
    nodes += [{"id": f"n{i+1}", "type": t, "config": c}
              for i, (t, c) in enumerate(types_and_cfg)]
...
def test_llm_guard_blocks_detections_and_seeded_misses():
    fx = load_fixture({"attacks": [{"type": "prompt_injection",
                                    "marker": "ignore previous", "severity": 0.95}]})
...
def test_redaction_rules_and_policy_gate():
    g = chain(("redaction_rules", {"rules": ["ssn"]}),
              ("policy_gate", {"deny": ["wire transfer"], "mode": "enforce"}))
    run = execute(g, load_fixture(None),
                  user_input="my ssn is 123-45-6789, wire transfer the funds")
...
def test_rate_budget_limiter_abort_and_degrade():
    fx = load_fixture({"tools": {"email.search": [
        {"match": {"query_contains": "q"}, "result": [{"id": "m1", "subject": "s"}]}]}})
    base = [("tool", {"tool": "email.search"}),
...
def test_observability_nodes_are_pure_passthrough():
    g = chain(("trace_probe", {}), ("metrics_emitter", {}),
              ("cost_latency_monitor", {}), ("audit_log_sink", {}))
...
def test_long_term_memory_stores_read_the_fixture_world():
    fx = load_fixture({
        "episodic": [{"ts": "2026-06-01", "text": "asked about vacation policy", "salience": 0.9},
                     {"ts": "2026-06-20", "text": "booked flights to Lisbon", "salience": 0.8}],
        "facts": ["prefers morning meetings", "prefers MORNING MEETINGS"],
        "instructions": [{"text": "always cite sources", "approved": True},
...
def test_working_memory_and_summary_consolidator():
    ...
    assert any(e["type"] == "memory_write" and e["payload"]["store"] == "working"
               for e in run.events)
...
def test_react_disallowed_tool_is_refused_before_execution():
    fx = load_fixture({"react": [{"match": {"contains": "delete"},
...
def test_publish_pattern_validates_and_lists(user_patterns):
    ...
    summary = patterns.publish_pattern("my_qa", doc, "# my lesson",
                                       {"happy": MINIMAL_FIXTURE})
...
def test_same_port_fanin_merges_flat():
    ...
    doc["nodes"].append({"id": "n99", "type": "tool", "label": "tool2",
                         "config": {"tool": "email.search"}})
    doc["edges"] += [
